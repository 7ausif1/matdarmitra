(() => {
  ...
  var FAQ = [
    ...
    },
    {
      ...
      a: "Indian EVMs are standalone (no internet/Bluetooth/Wi-Fi), one-time programmable, and sealed in multiple layers with candidate agents' signatures. EVMs are randomised twice before polling, stored in strongrooms with 24\xD77 CCTV, and the VVPAT paper trail allows physical verification. The Supreme Court has upheld the EVM+VVPAT system multiple times."
    },
    {
    ...
    <div class="mm-card glass mm-chat-window" style="padding: 0;">
      ...
      <div style="padding: 0 1.5rem 1rem; background-color: var(--color-paper); border-top: 1px solid var(--color-border);">
        <div style="display: flex; flex-wrap: wrap; gap: 0.5rem; margin-top: 1rem;" id="suggestions">
          ${["How do I register?", "What is VVPAT?", "Are EVMs secure?", "What is NOTA?"].map((s) => `<button class="mm-suggest-btn">${escapeHTML(s)}</button>`).join("")}
        </div>
      </div>
